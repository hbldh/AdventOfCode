...
def parse_stack(data):
    ...
    rows = [
        [
            pos.strip()
            for pos in textwrap.wrap(
                row.rstrip(),
                4,
                drop_whitespace=False,
            )
        ]
        for row in raw_stack.splitlines()
    ]
...
def parse_moves(data):
    ...
    moves = [
        tuple(int(t) for t in move.split(" ")[1::2]) for move in raw_moves.splitlines()
    ]
    return moves


def solve(data):
    return solve_part_1(parse_stack(data), parse_moves(data)), solve_part_2(
        parse_stack(data), parse_moves(data)
    )
