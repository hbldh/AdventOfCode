...
neighbours_4 = tuple(
    filter(
        lambda x: x != (0, 0, 0, 0),
        list(itertools.product([-1, 0, 1], [-1, 0, 1], [-1, 0, 1], [-1, 0, 1])),
    )
)
        ...
        for row_idx in range(y_start, y_end + 1):
            for column_idx in range(x_start, x_end + 1):
                ...
                for neighbour in neighbours_3:
                    n_activated_neighbours += int(
                        map[
                            (
                                column_idx + neighbour[0],
                                row_idx + neighbour[1],
                                layer_idx + neighbour[2],
                            )
                        ]
                        == "#"
                    )
            ...
            for row_idx in range(y_start, y_end + 1):
                for column_idx in range(x_start, x_end + 1):
                    ...
                    if map[(column_idx, row_idx, layer_idx, temporal_idx)] == ".":
                        if n_activated_neighbours == 3:
                            new_map[
                                (column_idx, row_idx, layer_idx, temporal_idx)
                            ] = "#"
                        else:
                            new_map[
                                (column_idx, row_idx, layer_idx, temporal_idx)
